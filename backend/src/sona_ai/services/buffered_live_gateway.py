...
class BufferedLiveGateway:
    """Decode a bounded rolling PCM window with an in-process transcriber."""
        ...
        *,
        ...
        if model not in self.supported_models:
            raise WhisperLiveInputError(
                f"This realtime gateway requires a {self.display_name} model."
            )
        ...
        try:
            ...
            raise WhisperLiveCapacityError(
                f"The realtime {self.display_name} session is busy. Try again shortly."
            ) from exc
        ...
        try:
            ...
            logger.warning("Realtime %s relay failed: %s", self.display_name, exc)
            raise WhisperLiveUnavailableError(
                f"Realtime {self.display_name} is unavailable. "
                "The recording can still be saved."
            ) from exc
        finally:
            if current_task is not None:
...
class BufferedLiveRelay:
    ...
    async def run(self) -> None:
        task_prefix = self.display_name.lower().replace(" ", "-")
        reader = asyncio.create_task(
            self._read_browser(),
            name=f"sona-{task_prefix}-live-reader",
        )
        decoder = asyncio.create_task(
            self._decode_audio(),
            name=f"sona-{task_prefix}-live-decoder",
        )
        ...
        while True:
            ...
            while True:
                ...
                if window_start > self.accumulator.commit_horizon + 0.05:
                    raise WhisperLiveUnavailableError(
                        f"Realtime {self.display_name} could not keep up. "
                        "The recording can still be saved."
                    )
    ...
    def _snapshot(self, *, final: bool) -> tuple[np.ndarray, float]:
        ...
        if final and self.right_context_samples:
            samples = np.pad(samples, (0, self.right_context_samples))
