...
def build_one_call_adaptive_summary_prompt(
    ...
    return (
        ...
        "- Treat the user instruction as the highest-priority formatting requirement.\n"
        "- If the user asks for a specific layout such as a table, checklist, or template, follow that layout instead of the default section format.\n"
        "- Use level-2 Markdown headings for every major section only when the user did not request a different layout.\n"
        "- Never use bold text as a section heading. Do not write **Section Heading**.\n"
        "- Put one blank line after every heading when headings are used.\n"
        "- Under each heading, write either one concise paragraph or 2-4 bullet points when using the default section format.\n"
        "- Do not mix loose paragraph text and bullets under the same heading unless necessary.\n"
        "- If the user asks for a table, output a valid GitHub-Flavored Markdown table with a header row and separator row.\n"
        "- For multiple points inside a table cell, separate them with <br>- item so the table remains valid Markdown.\n"
        "- Use bold text only inside paragraphs or bullets for important labels, names, or decisions.\n"
        "- Do not add an overall title.\n"
        "- Start directly with the requested summary content, not with a generic preamble.\n"
